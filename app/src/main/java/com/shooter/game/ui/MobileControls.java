...
public class MobileControls {
    ...
    public void handleTouch(Player player) {
        ...
        player.moveLeft = leftPressed; player.moveRight = rightPressed;
        player.jump = upPressed; player.jetpack = jetpackPressed || upPressed; player.shoot = shootPressed;
        float dx = 0, dy = 0;
        if (leftPressed) dx -= 1; if (rightPressed) dx += 1; if (upPressed) dy += 1; if (downPressed) dy -= 1;
        if (dx == 0 && dy == 0) dx = player.facingRight ? 1 : -1;
        float len = (float)Math.sqrt(dx * dx + dy * dy);
        if (len > 0) { player.aimDirX = dx / len; player.aimDirY = dy / len; }
    }
    public void render(ShapeRenderer sr) {
        sr.setColor(1, 1, 1, 0.3f); sr.rect(btnLeft.x, btnLeft.y, btnLeft.width, btnLeft.height);
        sr.rect(btnRight.x, btnRight.y, btnRight.width, btnRight.height);
        ...
        sr.setColor(0, 1, 1, 0.3f); sr.rect(btnJetpack.x, btnJetpack.y, btnJetpack.width, btnJetpack.height);
        sr.rect(btnGrenade.x, btnGrenade.y, btnGrenade.width, btnGrenade.height);
    }
}